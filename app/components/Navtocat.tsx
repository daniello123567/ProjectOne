...
function Navtocat() {
  const Box = ({imageSrc,Name,HoverImage}:ros)=>{
    ...
    <motion.img initial={{opacity:0.7}} exit={{opacity:0,transition:{duration:1}}} whileTap={{opacity:1}} whileHover={{opacity:1}} transition={{duration:.7}} className='w-full  hidden group-hover:opacity-100 group-hover:block h-full object-cover' src={`${HoverImage}`} alt={`${Name}`}/>
