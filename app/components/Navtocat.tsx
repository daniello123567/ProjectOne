...
function Navtocat() {
  const Box = ({imageSrc,Name,HoverImage}:ros)=>{
    ...
    <Image blurDataURL={imageSrc}  placeholder="blur" className='w-full absolute object-cover h-full transition-opacity duration-[1s] ease-in-out opacity-100 group-hover:opacity-0' width={500} height={500} src={`${imageSrc}`} alt={`${Name}`}/>
