...
function Navtocat() {
  const Box = ({imageSrc,Name,HoverImage}:ros)=>{
    ...
    <Image blurDataURL={imageSrc}  placeholder="blur" className='w-full transition-opacity duration-100 group-hover:hidden group-hover:opacity-0 h-full object-cover' width={500} height={500} src={`${imageSrc}`} alt={`${Name}`}/>
    <Image blurDataURL={imageSrc}  placeholder="blur" className='w-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black hidden group-hover:block h-full object-cover' width={500} height={500} src={`${HoverImage}`} alt={`${Name}`}/>
    ...
    <span className='w-[80%] group-hover:scale-x-100 scale-x-0 h-[.1em] bg-white absolute bottom-[.4em] origin-left transform  transition-transform duration-100 ease-out'></span>
    </div>
  }
  return (
