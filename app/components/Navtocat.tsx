...
function Navtocat() {
  const Box = ({imageSrc,Name,HoverImage}:ros)=>{
    ...
    <Image blurDataURL={imageSrc}  placeholder="blur" className='w-full transition-all duration-100 group-hover:hidden group-hover:opacity-0 h-full object-cover' width={500} height={500} src={`${imageSrc}`} alt={`${Name}`}/>
    <Image blurDataURL={imageSrc}  placeholder="blur" className='w-full opacity-0 group-hover:opacity-100 transition-all duration-300 bg-black hidden group-hover:block h-full object-cover' width={500} height={500} src={`${HoverImage}`} alt={`${Name}`}/>
