...
function Navtocat() {
  const Box = ({imageSrc,Name,HoverImage}:ros)=>{
    ...
    <motion.img initial={{opacity:0.7}} whileHover={{opacity:1}} transition={{duration:.6}} className='w-full opacity-0 group-hover:opacity-100 transition-all duration-300 bg-black hidden group-hover:block h-full object-cover' src={`${HoverImage}`} alt={`${Name}`}/>
